...
group by 1,2
order by 1,2
--//Write a query to return the minimum and maximum spend from all the customers in June 2020 :
with tmp as (select 
customer_id,
...
select
min(total_spend) as min,
max(total_spend) as max
from 
tmp
--//Write a query to return the average spend per customer per month :
with tmp as (
  select
  extract(year from payment_ts ) as year ,
  extract(month from payment_ts ) as month ,
